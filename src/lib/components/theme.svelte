...
<button
  class="{dark
    ...
    : 'bg-yellow-100 focus:ring-yellow-400 ring-offset-white border-[1px] border-slate-200'} relative inline-flex flex-shrink-0 h-5 w-9 border-2 border-transparent rounded-full cursor-pointer transition-colors ease-in-out duration-200 focus:outline-none focus:ring-2 focus:ring-offset-2"
  class:hidden
  type="button"
  aria-checked={dark}
  on:click={toggle}
